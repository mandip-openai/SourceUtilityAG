...
#### 2\. Timeliness and timelessness Evaluation

Prompt:

"Assess the timeliness of the given content based on its relevance to the topic and user expectations. Determine if the information is up-to-date, considering the following factors:
Query Intent & Content Relevance 
    * Does the topic require frequent updates (e.g., news, finance, health, technology), or is it more stable over time (e.g., historical facts, fundamental theories)?
Freshness for Evolving Topics 
    * Is the content recent, regularly updated, and aligned with ongoing developments?
Indicators of Timeliness 
    * Check publication dates, update timestamps, references to recent studies or events.
Timeless vs. Time-Sensitive Information
    * Does the content remain valid and authoritative, or has it become outdated? If content remains valid irrespective of how fresh it is, it should be rated high.
Trustworthiness & Timeliness
    * Is the information reliable based on its last update, or could outdated content mislead users?

Instruction: Provide a rating in the scale 1-7 with a brief justification based on these criteria. If the content does not require frequent updates or timeless then it needs to be default to High (6-7).

Evaluation Levels: Unless specified otherwise, use the following scale to classify evaluation levels: 
    * Low (1-2): Content is outdated, irrelevant, or missing key updates for a time-sensitive topic.
    * Medium (3-5): Content is somewhat current but may lack the latest updates. 
    * High (6-7): Content is recent, frequently updated, and relevant to evolving topics. Timeless or stable content should also be classified as High.
...
def main(
    ...
    output_path: str = "/tmp/dump_feather_results/graded_search_result_groups_dump_Rel_SamplewC_timeliness_nv4_wC.jsonl",
