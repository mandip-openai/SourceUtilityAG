...
DEFAULT_RUBRICS = r"""
To evaluate search results effectively, think like a professional rater using a structured approach. Your goal is to assess each result’s quality based on its ability to meet user needs, focusing on the following key factors that you are asked below on Relevance. Follow these instructions:
...
* Low (1-2): Indicates minimal relevance or poor alignment with the query. The result does not address the query or provides irrelevant, generic, or misleading content.
* Medium (3-5): Reflects partial relevance, with some alignment to the query but lacking depth or focus. The result partially addresses the query, providing some relevant information but lacking depth, specificity, or contextual alignment.
* High (6-7): Signifies strong relevance and clear alignment with the query intent, providing comprehensive and precise information. The result directly addresses the query’s intent, is highly specific, and aligns with the query's context. The information provided is complete, useful, and easy to understand.
...
def main(
    ...
    output_path: str = "/tmp/dump_feather_results/graded_search_result_groups_dump_Rel_SamplewC_nv4_wC.jsonl",
    model: TModelType = "nv4",
    rating_mode: TRatingMode = "snippet_wo_context",
    limit: int | None = None,
    grading_concurrency: int = 10,
    random_seed: int | None = None,
    read_buffer: int | None = None,
