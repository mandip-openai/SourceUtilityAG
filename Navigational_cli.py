...
#### 3\. Navigational or Transactional Evaluation

Prompt:

"Assess the quality of the search result based on its ability to meet the user's navigational or transactional intent. Determine if the result provides value and matches the specific needs of the user.
Evaluation Criteria:

Navigational Intent Definition: The user is seeking a specific website or URL, intending to navigate directly to it.
    * High: The result is the exact website or URL the user is looking for, offering direct access without the need for further searching.
    * Medium: The result is relevant and could provide value to the user by offering closely related content or alternative options of interest.
    * Low: The result is unrelated or not beneficial for the user's specific navigational goal, providing no meaningful connection to the desired website.

Transactional Intent Definition: The user intends to complete a transaction, such as making a purchase, booking a service, or finding product information.
    * High: The result enables the user to complete the intended transaction efficiently, providing clear paths to purchase, booking, or inquiry.
    * Medium: The result offers helpful information or options related to the transaction, which could aid the user in making a decision but may not be the final transactional destination.
    * Low: The result does not assist in completing the transaction, lacking relevant information or actionability.

Instruction: Provide a rating in the scale of 1-7 with a brief justification based on these criteria

Evaluation Levels: Unless specified otherwise, use the following scale to classify evaluation levels: 
    * Low (1-2): Fails to meet the user's intent, offering little to no value in relation to the query.
    * Medium (3-5):  Provides relevant information or options but may require additional steps or decisions.
    * High (6-7): Clearly meets navigational or transactional intent, offering direct value and fulfilling user needs.
...
def main(
    ...
    output_path: str = "/tmp/dump_feather_results/graded_search_result_groups_dump_Rel_SamplewC_NavTran_nv4_woC.jsonl",
